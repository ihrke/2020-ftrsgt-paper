...
report.filename <- function(report.fname, base=NULL){
  if(is.null(base))
    bname<-tools::file_path_sans_ext(basename(this.file.name()))
  else
    bname<-tools::file_path_sans_ext(basename(base))
  
  dir.create(file.path('reports', bname), showWarnings = FALSE)
  fname<-file.path('reports', bname, report.fname)
  cat("> ", fname, "\n")
  fname  
}


plot.filename <- function(plot.name, base=NULL){
  if(is.null(base))
    bname<-tools::file_path_sans_ext(basename(this.file.name()))
  else
    bname<-tools::file_path_sans_ext(basename(base))
  
  dir.create(file.path('graphs', bname), showWarnings = FALSE)
  fname<-file.path('graphs', bname, plot.name)
  cat("> ", fname, "\n")
  fname  
}
